...
function WaitlistForm() {
  ...
  const handleSubmit = useCallback(async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setToast({
        message:
          result.mode === "preview"
            ? "Preview saved for this browser session. Connect the launch endpoint before publishing."
            : result.mode === "duplicate"
              ? "You’re already on the list — we’ll be in touch at launch."
              : "You’re on the list — Zama will be in touch soon.",
  ...
  return (
    <>
      ...
      >
      <p className="basis-full px-1 text-xs font-bold uppercase tracking-[0.1em] text-brand-orange-ink">{waitlistEyebrow}</p>
      ...
      {toast ? <Toast message={toast.message} tone={toast.tone} onDismiss={handleDismissToast} /> : null}
    </>
  );
}
      ...
      <div className={`hero-layout relative z-[1] grid min-h-auto grid-cols-1 items-center gap-5 py-7 sm:py-9 lg:min-h-[min(650px,calc(100vh-108px))] lg:grid-cols-[minmax(0,1.06fr)_minmax(380px,0.94fr)] lg:gap-2 lg:py-11 ${sectionShell}`}>
        <div className="hero-content relative z-[2] flex min-w-0 flex-col items-start gap-4">
          ...
          <h1 id="hero-title" className="hero-h1 max-w-full text-balance font-primary text-[clamp(2.2rem,11.5vw,3.35rem)] font-bold leading-[0.94] tracking-[-0.035em] text-brand-warm-white sm:max-w-[11ch] sm:text-[clamp(4rem,8vw,5.6rem)] lg:text-[clamp(4.2rem,5.8vw,5.4rem)]">
            ...
            <span className="hero-highlight relative inline-block text-brand-yellow">{hero.titleHighlight}</span>
            <span className="block">{hero.titleB}<span className="inline-block rotate-6 text-brand-orange">{hero.exclaim}</span></span>
          </h1>
        ...
        <div className="hero-media hero-media-shell relative z-[1] grid min-h-80 min-w-0 place-items-center sm:min-h-105 lg:min-h-125" aria-label={hero.mediaAlt}>
          <span className="absolute top-5 right-1 z-[3] rotate-3 rounded-wobbly-tag border-2 border-dashed border-brand-yellow bg-brand-warm-white px-3 py-2 text-xs font-bold uppercase tracking-[0.08em] text-brand-forest shadow-brand sm:right-4">{hero.readyBadge}</span>
          <div className="relative z-[2] w-[106%] max-w-none -translate-x-1 -rotate-1 sm:w-[122%] sm:-translate-x-10 sm:-rotate-2 lg:w-[132%] lg:-translate-x-12">
            <img className="hero-product h-auto w-full object-contain" src={hero.heroImage} alt={hero.heroImageAlt} width="612" height="408" fetchPriority="high" />
          </div>
          <div className="harvest-note absolute bottom-5 left-0 z-[3] grid rotate-[-3deg] gap-0.5 rounded-[20px_28px_18px_24px/26px_18px_28px_20px] border-3 border-brand-forest bg-brand-warm-white px-4 py-3 text-brand-black shadow-brand sm:left-2">
            <strong className="font-primary text-xl leading-none text-brand-orange-ink">{hero.harvestTitle}</strong>
            <span className="text-xs leading-[1.15]">{hero.harvestCopy}</span>
          </div>
        </div>
      </div>
    </section>
