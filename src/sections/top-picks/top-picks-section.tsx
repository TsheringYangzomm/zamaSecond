...
type TopPick = { name: string; duration: string; tone: "white" | "yellow" };

function DishCard({ name, duration, tone }: TopPick) {
  return (
...
export function TopPicksSection() {
  ...
  const topPicks = blocks.topPicks;
  ...
  return (
    <section className={`deferred-section top-picks grid grid-cols-1 items-center gap-[1.3rem] py-[clamp(1rem,3vw,1.8rem)] sm:grid-cols-[minmax(220px,0.52fr)_1fr] ${sectionShell}`} aria-labelledby="top-picks-title">
      <div className="top-picks-copy top-picks-copy-shell relative">
        <OutlineTag>{topPicks.tag}</OutlineTag>
          <h2 id="top-picks-title" className="font-primary text-[clamp(2rem,3.5vw,3.5rem)] font-bold leading-[1.02] text-brand-green-ink">{topPicks.heading}</h2>
      </div>
      <div className="dish-grid grid grid-cols-1 gap-[0.9rem] sm:grid-cols-3">
        {topPicks.items.map((pick) => <DishCard key={pick.name} {...pick} />)}
      </div>
    </section>
  );
}
