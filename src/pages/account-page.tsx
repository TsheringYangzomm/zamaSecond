...
function AccountDashboard({ profile }: { profile: CustomerProfile }) {
  ...
  const [orderFilter, setOrderFilter] = useState<OrderFilter>(() => typeof window !== "undefined" && new URLSearchParams(window.location.search).get("section") === "returns" ? "returns" : "all");
...
export function AccountOrdersPage() {
  ...
  useEffect(() => {
    const syncReturnSection = () => {
      if (window.location.pathname !== "/account/orders" || new URLSearchParams(window.location.search).get("section") !== "returns") return;
      setOrderFilter("returns");
      setReviewingOrder(null);
      setReturningOrder(null);
      requestAnimationFrame(() => document.getElementById("return-center")?.scrollIntoView({ behavior: "smooth", block: "start" }));
    };
    window.addEventListener("popstate", syncReturnSection);
    syncReturnSection();
    return () => window.removeEventListener("popstate", syncReturnSection);
  }, []);
