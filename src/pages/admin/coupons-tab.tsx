...
import { deactivateAdminCoupon, generateCouponCode, listAdminCoupons, saveAdminCoupon, type AdminCouponsResult } from "../../coupons/coupons-api";
...
import { Checkbox, Field, RequiredMark, inputClasses, selectClasses, textAreaClasses } from "./admin-fields";
...
function CouponForm({ initial, products, categories, busy, error, onSave, onCancel }: { initial: CouponAdminDraft; products: ProductRow[]; categories: string[]; busy: boolean; error: string | null; onSave: (draft: CouponAdminDraft) => void; onCancel: () => void }) {
  ...
  return (
    <div className="grid gap-5"><div className="flex flex-wrap items-center justify-between gap-3"><div className="grid gap-1"><span className="text-xs font-bold uppercase tracking-[0.12em] text-brand-orange-ink">Commerce</span><h1 className="font-primary text-[clamp(1.7rem,3.5vw,2.4rem)] font-bold leading-[1.02] text-brand-green-ink">{initial.id ? "Edit coupon" : "New coupon"}</h1><p className="text-sm text-brand-black/68">Set the offer, its audience, and the dates when it can be used.</p></div><button className={btnOutlineSm} type="button" onClick={onCancel}>← Back to coupons</button></div>
      <form className="grid gap-5 rounded-wobbly-card border-3 border-brand-forest bg-brand-white p-5 shadow-brand-soft" onSubmit={(event) => { event.preventDefault(); onSave(draft); }}>
        <div className="grid gap-3 sm:grid-cols-2"><Field label="Coupon code" required hint="Generated automatically. You can regenerate it before saving." htmlFor="coupon-code"><div className="flex gap-2"><input className={`${inputClasses} min-w-0 flex-1`} id="coupon-code" required value={draft.code} onChange={(event) => update("code", event.target.value.toUpperCase())} /><button className={btnOutlineSm} type="button" onClick={() => update("code", generateCouponCode())}>Generate new</button></div></Field><Field label="Campaign title" required htmlFor="coupon-title"><input className={inputClasses} id="coupon-title" required value={draft.title} onChange={(event) => update("title", event.target.value)} placeholder="Fresh start" /></Field></div>
        ...
        <div className="grid gap-3 sm:grid-cols-3"><Field label="Discount type" required htmlFor="coupon-discount-type"><select className={selectClasses} id="coupon-discount-type" required value={draft.discountType} onChange={(event) => update("discountType", event.target.value as CouponAdminDraft["discountType"])}><option value="percentage">Percentage</option><option value="fixed">Fixed amount</option></select></Field><Field label={draft.discountType === "percentage" ? "Discount percentage" : "Discount amount"} required htmlFor="coupon-discount-value"><input className={inputClasses} id="coupon-discount-value" type="number" min="0.01" max={draft.discountType === "percentage" ? 100 : undefined} step="0.01" required value={draft.discountValue} onChange={(event) => update("discountValue", Number(event.target.value))} /></Field><Field label="Maximum discount" hint="Optional cap for percentages." htmlFor="coupon-max-discount"><input className={inputClasses} id="coupon-max-discount" type="number" min="0.01" step="0.01" placeholder="No cap" value={draft.maximumDiscountAmount ?? ""} onChange={(event) => update("maximumDiscountAmount", event.target.value ? Number(event.target.value) : null)} disabled={draft.discountType !== "percentage"} /></Field></div>
